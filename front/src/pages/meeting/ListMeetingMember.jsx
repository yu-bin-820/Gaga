...
const ListMeetingMember = () => {
   ...
   const {data : pendingMemberList, mutate : mutatePendingMemberList} = useSWR(`http://${import.meta.env.VITE_SPRING_HOST}/rest/user/list/grouptype/2/no/${meetingno}/state/1`,fetcher)
   const {data : confirmedMemberList, mutate : mutateConfirmedMemberList} = useSWR(`http://${import.meta.env.VITE_SPRING_HOST}/rest/user/list/grouptype/2/no/${meetingno}/state/2`,fetcher)
    ...
    return (
        ...
        <Button id={pendingMember.userNo} onClick={onClickDeleteMember}>거절 </Button>
        <Button id={pendingMember.userNo} onClick={onClickUpdateMember}>수락 </Button>
        </Box>
        ))}
        <h5>확정 멤버</h5>
        {confirmedMemberList?.map((confirmedMember,i)=>(
            ...
            <Button id={confirmedMember.userNo} onClick={onClickDeleteMember}>내보내기 </Button>
            </Box>
            ))}
        </Box>
        </>
