...
const ListMyMeeting = () => {
    ...
    return (
        <Box sx={{ marginBottom: '130px', backgroundColor: '#ededed' }}>
            ...
            {meetingList?.filter(meeting => meeting.state === 2 && meeting.meetingSuccess === 1&& meeting.meetingState === (1||2)).length === 0 && (
                <NoMeeting 
                ment={'참여가 확정된 모임이 없습니다'}/>
            )}

            {meetingList?.map((meeting, i) => {
                if (meeting.state === 2 && meeting.meetingSuccess === 1 && meeting.meetingState === (1||2)) {
                return (
            ...
            {meetingList?.filter(meeting => meeting.state === 1 && meeting.meetingSuccess===1 && meeting.meetingState === (1||2)).length === 0 && (
                <NoMeeting 
                ment={'참여 신청한 모임이 없습니다'}/>
            )}

            {meetingList?.map((meeting, i) => {
                if (meeting.state === 1 && meeting.meetingSuccess===1  && meeting.meetingState === (1||2)) {
                return (
                    <Box key={i} sx={{ margin: '3px' }}>
                    {!isMyProfile&&(<ListMeetingProfile meeting={meeting} />)}
            ...
            {meetingList?.filter(meeting => meeting.state === 0 && meeting.meetingState === (1||2||3)).length === 0 && (
                <NoMeeting 
                ment={'주최한 모임이 없습니다'}/>
            )}

            {meetingList?.map((meeting, i) => {
                if (meeting.state === 0 && meeting.meetingState === (1||2||3)) {
                return (
                    <Box key={i} sx={{ margin: '3px' }}>
                    {!isMyProfile&&(<ListMeetingProfile meeting={meeting} />)}
            ...
            {meetingList?.filter(meeting => meeting?.state === ( 2) && meeting?.meetingSuccess === 2& meeting?.meetingState ===(2||3)).length === 0 && (
                <NoMeeting 
                ment={'성사된 모임이 없습니다'}/>
            )}

            {meetingList?.map((meeting, i) => {
                if (meeting?.state === ( 2 || 3) && meeting?.meetingSuccess === 2 && meeting?.meetingState ===(2||3)) {
                return (
                    <Box key={i} sx={{ margin: '3px' }}>
